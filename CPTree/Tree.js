...
class Tree extends React.Component {
    ...
    componentDidMount() {
        this.props.setScrollToEnd(this.scrollToEnd);
    }
    componentWillMount() {
        let {
            checkable,
    ...
    renderItem(node) {
        ...
        const {key, children, icon, name, disabled} = node;
        ...
        let selectColor = '#fff';
        let textStyle = {fontSize: scale(40), fontFamily:Platform.OS==='ios'?'IRANSansMobile-Light' : 'IRANSansMobile_Light'};
        if (disabled) {
            selectColor = '#D0D0D0';
        ...
        if (typeof name === 'string') {
            textNode = <Text style={[textStyle, nodeStyle]}>{name} </Text>
        } else {
            textNode = <View>{name}</View>
        }
        ...
        return (
            <View style={styles.item}>
                ...
                <Icon
                    ...
                    style={[styles.icon, {color: '#000', paddingLeft: 5}]}
                    size={iconSize}
                    name={selectIcon}
                />}
                {icon &&
    ...
    scrollToEnd = ()=> this.scroll.scrollToEnd()
    render() {
        let styles = this.props.styles || defaultStyles;
        return <ScrollView
            ref={(scroll) => {
